...
static Future<DetailMovie> getDetailMovie(Movie movie, {http.Client client}) async {
  String url = "http://api.themoviedb.org/3/movie/${movie.id}?api_key=$apiKey&language=en-US";
...
return DetailMovie(movie, language: language,genres: genres.map((e) => (e as Map<String, dynamic>)['name'].toString()).toList());
}

static Future<List<CreditMovie>> getCredits(int movieID, {http.Client client}) async {
  String url = "http://api.themoviedb.org/3/movie/$movieID/credits?api_key=$apiKey";
